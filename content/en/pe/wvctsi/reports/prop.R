...
# Prop flat -----
prop <- read_csv(here(data, "prop.csv")) %>%
  separate(Cores, 
  ...
  mutate(tip = case_when(
    str_detect(Type, "Industry") ~ paste0(Amounts," were proposed to an ", Type, " by ", Core),
    str_detect(Type, "Other") ~ paste0(Amounts, " were proposed by ",  Core, " to an ", Type," agency"),
    TRUE ~ paste0(Amounts, " were proposed by ",  Core, " to a ", Type," agency")
  )
  ) %>%
  mutate(tip = str_wrap(tip, 10))

# Plot awards ----
prop_plot <- ggplot(data = prop, 
...
prop_norepel <- gginnards::delete_layers(prop_plot + guides(fill=FALSE), 
                                          match_type ="GeomLabelRepel")

# Prop int -----
girafe(code = print(prop_norepel),
       width = 7, 
       options = list(
