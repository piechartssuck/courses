...
While proprietary softwares such as SPSS, SAS, Minitab, etc. may be easier to learn, `R` and others like Python are free, open, and widley used. Picking on SPSS, as of this writing users pay for different tiers depending on needs (such as machine learning, methods to deal with missing data, etc.) they want to use.
...
some_ed_data <- 
  read_csv(here::here("static", "data", "raw", "some_ed_data.csv"))
```
...
```{r echo=FALSE, message = FALSE, purl=FALSE}
some_exercise_data <- 
  read_csv(here::here("static", "data", "raw", "some_exercise_data.csv"))
```
...
```{r echo=FALSE, message = FALSE, purl=FALSE}
some_survey_data <- 
  read_csv(here::here("static", "data", "raw", "some_survey_data.csv"))
```
